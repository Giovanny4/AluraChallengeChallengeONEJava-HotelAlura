...
public class RegistroHuesped extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RegistroHuesped frame = new RegistroHuesped(new Reserva());
...
//	}
	...
	private void guardar() {
		...
		String fechaNacimientoS = ((JTextField) txtFechaN.getDateEditor().getUiComponent()).getText();
		Date fechaNacimiento = java.sql.Date.valueOf(fechaNacimientoS);
		String nacionalidad = txtNacionalidad.getSelectedItem().toString();
		String telefono = txtTelefono.getText();
		Integer id = reserva.getId();
		
		Huesped huesped = new Huesped(nombre, apellido, fechaNacimiento, nacionalidad, telefono, id);
		this.huespedController.guardar(huesped);

		JOptionPane.showMessageDialog(this, "Su número de reserva es :" + id);
	}
}
